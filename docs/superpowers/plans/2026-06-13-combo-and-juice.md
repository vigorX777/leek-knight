...
**Goal:** 为 Stonk Rider 增加双 combo 系统、真实跳跃、保留金币、牵引力渐变和结算爆发力，让玩法从"被动看 K 线"升级为"主动操作躲避/追逐"。
...
| `src/game/StonkGame.ts` | `GameCallbacks` 扩展 `onComboBreak`、`onSound` 加 combo | 修改 |
...
### Task 9: StonkGame 和 RideSceneOptions 接口扩展

**Files:**
- Modify: `src/game/StonkGame.ts`
- Modify: `src/game/RideScene.ts`

- [ ] **Step 1: 扩展 `GameCallbacks` 接口**

编辑 `src/game/StonkGame.ts` 的 `GameCallbacks` 接口（约第 5–12 行）：
...
```bash
git add src/game/StonkGame.ts src/game/RideScene.ts
...
在 `new StonkGame(...)` 的选项对象中，`onSound` 之后新增：
...
Task 8 (RideScene particles)   ← 定向粒子
    ↓
Task 9 (StonkGame + 接口)      ← 修复接口不匹配（串行瓶颈）
    ↓
